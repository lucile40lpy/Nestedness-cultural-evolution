...
cultural_evolution <- function(
    ...
    set_transmission, interaction_rate, mutation_rate,
  ...
  # If no transmission
    ...
    ### No transmission (mutations only) ----
    for (gen in 1:burn_in) {
      for (agent in 1:num_agents) {
        for (item in 1:num_items) {
          if (set_appeal) {
            # Use appeal score as mutation rate
            if (runif(1) < appeal_scores[item]) {
              if (binary_matrix[agent, item] == 0) {
                if (item == 1) {
                  binary_matrix[agent, item] <- 1
                } else {
...
### A. Parameters lists ----
matrix_size <- list(c(50,20), c(100, 40), c(500, 200))
num_gen <- c(100, 200, 500)
...
gen_labels <- paste0(seq(10, 500, 10))
